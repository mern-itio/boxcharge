...
import { listMedia, deleteMedia, recordMedia } from "@/lib/cms.functions";
...
function MediaLibrary() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      await uploadCmsImage(file, recordFn, alt || undefined);
      qc.invalidateQueries({ queryKey: ["cms", "media"] });
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((m) => (
          <div key={m.id} className="rounded-xl border border-border/60 bg-card/30 p-2">
            ...
            <div className="mt-2 truncate text-xs">{m.filename}</div>
